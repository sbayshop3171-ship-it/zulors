<template>
    <section class="relative h-full snap-start snap-always overflow-hidden bg-black text-white">
        ...
        <div class="absolute left-4 right-20 bottom-5 z-20">
            ...
            <a v-if="adData.cta_type !== 'NO_BUTTON'" v-bind:href="adData.click_url || adData.target_url" target="_blank" rel="noopener" class="pointer-events-auto mt-3 inline-flex rounded-xl bg-white px-4 py-2.5 text-par-s font-bold text-black">
                {{ adData.cta_text }}
            </a>
        </div>
