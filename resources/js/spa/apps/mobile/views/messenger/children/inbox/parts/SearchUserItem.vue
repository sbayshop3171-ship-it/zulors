<template>
	<div class="flex items-center gap-3 px-4 py-3 active:bg-fill-qt">
		<button type="button" class="flex items-center gap-3 flex-1 min-w-0 text-left" v-on:click="$emit('select', userData)">
			...
			<span class="block flex-1 min-w-0">
				<Name v-bind:name="userData.name" v-bind:isVerified="userData.verified"></Name>
		...
		<button v-if="removable" type="button" class="inline-flex-center size-small-avatar rounded-full bg-fill-qt text-lab-tr shrink-0" v-on:click.stop="$emit('remove', userData)">
...
<script>
	...
	import Name from '@M/components/people/Name.vue';
