<template>
    <div v-on:click="lightboxImages" class="w-full max-w-[68vw] overflow-hidden rounded-2xl bg-fill-tr p-1 cursor-pointer group sm:max-w-72">
        <img v-on:error="onImageError" v-bind:src="imageUrl" v-bind:alt="mediaData.userName" class="block w-full h-auto rounded-xl smoothing group-hover:opacity-90 transition-all">
    </div>
</template>
