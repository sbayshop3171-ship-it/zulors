...
class ReconcileMediaPublications extends Command
{
    ...
    public function handle(): int
    {
        ...
        $publications = MediaPublication::where(function ($query) {
            $query->whereNotIn('status', ['published', 'cancelled'])
                ->orWhere(function ($published) {
                    $published->where('status', 'published')
                        ->where('has_video', true)
                        ->whereHas('items', fn($items) => $items->where('type', 'video')->whereIn('status', ['uploaded', 'processing']));
                });
        })
        ...
        foreach($publications as $publication) {
            DB::transaction(function () use ($publication) {
                ...
                if($publication->status === 'published') {
                    $this->recoverLostItemDispatches($publication);
                    $publication->touch();
                    return;
                }
    ...
    private function recoverLostItemDispatches(MediaPublication $publication): void
    {
